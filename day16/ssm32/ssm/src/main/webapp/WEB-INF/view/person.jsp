...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      <span class="input-group-addon">人员 姓名</span>
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
			      <span class="input-group-addon">区间查询</span>
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reservePerson.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>人员姓名：</td>
								<td><input type="text" id="pName" name="pName"class="form-control" aria-required="true" required/></td>
								<td>年龄：</td>
								<td><input type="text" id="pAge" name="pAge"class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>性别：</td>
								<td><input type="radio" id="pSex" name="pSex" value="男" aria-required="true" required/>男
									<input type="radio" id="pSex" name="pSex" value="女" aria-required="true" required/>女</td>
								<td>日期：</td>
								<td><input type="date" id="createtime" name="createtime"class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>人员爱好：</td>
								<td>
									<input type="checkbox" id="pLike1" name="pLike"  aria-required="true" required value="篮球"/>篮球
									<input type="checkbox" id="pLike2" name="pLike"  aria-required="true" required value="游泳"/>游泳
								</td>							
								<td>存储金额：</td>
								<td><input type="text" id="pCount" name="pCount"class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>银行名称：</td>
								<td colspan="4">
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
						<form id="personId" action="importPerson.htm" method="post" enctype="multipart/form-data">
							<input type="file" name="personXls" id="personXls">
						</form>
				</div>
				<div class="modal-footer">
	...
	$(function () {
	  ...
	  //导入
    var validator = $("#personId").validate({
		submitHandler: function(form){
		      $(form).ajaxSubmit({
		    	dataType:"json",
		    	success: function (data) {
	    		    	...
	    		    	//报表向后端发送ajax	
	    		    			$.ajax({
	    		    			    ...
	    		    			    success:function(res){
	    		    			    	var xdata = new Array();
	    		    			    	var ydata = new Array();
	    		    			    	res.tlist.forEach(function(personVo){
	    		    		    				xdata.push(personVo.cname);
	    		    			    			ydata.push({value:personVo.num,name:personVo.cname})
	...
	function initEditUser(getSelection){
		...
		$("#pLike1").prop("checked",getSelection.pLike.indexOf("篮球")!=-1);
		$("#pLike2").prop("checked",getSelection.pLike.indexOf("游泳")!=-1);
