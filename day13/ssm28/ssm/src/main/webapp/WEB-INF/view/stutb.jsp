...
<body class="gray-bg">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveStutb.htm">
						<input type="hidden" name="sId" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>学生姓名：</td>
								<td><input type="text" id="sName" name="sName"class="form-control" aria-required="true" required/></td>

								<td>性别：</td>
								<td><input type="radio" id="sSex" name="sSex"value="男" aria-required="true" required/>男
								<input type="radio" id="sSex" name="sSex"value="女" aria-required="true" required/>女</td>
							</tr>
							<tr>
								<td>学生爱好：</td>
								<td>
									<input type="checkbox" id="sHobby1" name="sHobby"  aria-required="true" required value="篮球"/>篮球
									<input type="checkbox" id="sHobby2" name="sHobby"  aria-required="true" required value="游泳"/>游泳
								</td>		
						
								<td>生日：</td>
									<td><input type="date" id="sBirth" name="sBirth"class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>角色：</td>
								<td colspan="4">
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
						<form id="stutbId" action="importStutb.htm" method="post" enctype="multipart/form-data">
							<input type="file" name="stutbXls" id="stutbXls">
						</form>
				</div>
				<div class="modal-footer">
	...
	$(function () {
	  ...
	  //导入
    var validator = $("#stutbId").validate({
		submitHandler: function(form){
		      $(form).ajaxSubmit({
		    	dataType:"json",
		    	success: function (data) {
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	//报表弹框
	    		    	...
	    		    	//报表向后端发送ajax	
	    		    			$.ajax({
	    		    			    url:"echartsStutb.htm",
	    		    			    dataType:"json",
	    		    			    type:"post",
	    		    			    success:function(res){
	    		    			    	var xdata = new Array();
	    		    			    	var ydata = new Array();
	    		    			    	res.tlist.forEach(function(StutbVo){
	    		    		    				xdata.push(StutbVo.cname);
	    		    			    			ydata.push({value:StutbVo.num,name:StutbVo.cname})
	...
	function initEditUser(getSelection){
		...
		$("#sHobby2").prop("checked",getSelection.sHobby.indexOf("游泳")!=-1);
