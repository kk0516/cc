...
<body class="gray-bg">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveStudentt.htm">
						<input type="hidden" name="id" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>用户名：</td>
								<td><input type="text" id="name" name="name"class="form-control" aria-required="true" required/></td>
								<td>年龄</td>
								<td><input type="text" id="age" name="age"class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>编码：</td>
								<td><input type="text" id="code" name="code"class="form-control" aria-required="true" required/></td>
								<td>所属年级:</td>
								<td colspan="4">
							...
							<tr>
								<td>入校时间：</td>
								<td><input type="date" id="entrytime" name="entrytime"class="form-control" aria-required="true" required/></td>
								<td>创建时间</td>
	...
	<div class="modal fade" id="modal_user_echarts" role="dialog" aria-labelledby="modal_user_echarts" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
				<div id="main" style="wdith:600px;height:300px">
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
						<form id="studenttId" action="importStudentt.htm" method="post" enctype="multipart/form-data">
							<input type="file" name="studenttXls" id="studenttXls">
						</form>
				</div>
				<div class="modal-footer">
	...
	$(function () {
	  ...
	  //导入
    var validator = $("#studenttId").validate({
		submitHandler: function(form){
		      $(form).ajaxSubmit({
		    	dataType:"json",
		    	success: function (data) {
	    		    		...
	    		    		//发送jaxa
	    		    			$.ajax({
	    		    			    ...
	    		    			    success:function(res){ 
	    		    			    	...
	    		    			    	var ydata=new Array();
	    		    			    	//循坏后台设置到前台的值
	    		    			    	 res.list.forEach(function(person){
	    		    			    		 xdata.push(person.sname);
	    		    			    		 ydata.push({value:person.num,name:person.sname});
	    		    			    	 });
	    		    			    	alert(xdata)
	...
	$("#export_user_btn").click(function(){
		//往后端发送请求
		location.href="exportStudentt.htm";	
	$('#modal_user_del').modal('hide');   		
	});
