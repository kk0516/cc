				...
				<div class="input-group">
					...
					<select class="form-control" name="mid" id = "txt_search_roleid">
						...
						<c:forEach items="${clist }" var="c">
						 	<option value="${c.mdId }">${c.mdname}</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveStuday.htm">
						<input type="hidden" name="sdId" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>学生姓名：</td>
								<td><input type="text" id="sdname" name="sdname"class="form-control" aria-required="true" required/></td>
								<td>学生性别：</td>
								<td><input type="radio" id="sdsex" name="sdsex" value="男" aria-required="true" required/>男
								<input type="radio" id="sdsex" name="sdsex" value="女" aria-required="true" required/>女</td>
							</tr>
							<tr>
								<td>学生爱好：</td>	
								<td>
									<input type="checkbox" id="sdhobby1" name="sdhobby"  aria-required="true" required value="篮球"/>篮球
									<input type="checkbox" id="sdhobby2" name="sdhobby"  aria-required="true" required value="游泳"/>游泳
								</td>
								<td>学生生日：</td>
								<td><input type="date" id="sdbirth" name="sdbirth" class="form-control" aria-required="true" required/></td>
							</tr>			
							<tr>
								<td>专业：</td>
								<td colspan="4">
...
<!--报表对话框 -->
	<div class="modal fade" id="modal_user_echarts" role="dialog" aria-labelledby="modal_user_echarts" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					 <h4 class="modal-title" id="modal_user_echar_head"> 报表  </h4>
				</div>
				<div class="modal-body">
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
						<form id="studayId" action="importStuday.htm" method="post" enctype="multipart/form-data">
							<input type="file" name="StudayXls" id="StudayXls">
						</form>
				</div>
				<div class="modal-footer">
	...
	$(function () {
	  ...
	  //导入
    var validator = $("#studayId").validate({
		submitHandler: function(form){
		      $(form).ajaxSubmit({
		    	dataType:"json",
		    	success: function (data) {
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	//报表弹框
	    		    	...
	    		    	//报表向后端发送ajax	
	    		    			$.ajax({
	    		    			    url:"echartsStuday.htm",
	    		    			    dataType:"json",
	    		    			    type:"post",
	    		    			    success:function(res){
	    		    			    	var xdata = new Array();
	    		    			    	var ydata = new Array();
	    		    			    	res.tlist.forEach(function(studayVo){
	    		    		    				xdata.push(studayVo.dname);
	    		    			    			ydata.push({value:studayVo.num,name:studayVo.dname})
	...
	function initEditUser(getSelection){
		...
		$("input[value='"+getSelection.sdsex+"']").prop("checked",true);//性别     勤劳，耐心，负责
		$("#sdhobby1").prop("checked",getSelection.sdhobby.indexOf("篮球")!=-1);
		$("#sdhobby2").prop("checked",getSelection.sdhobby.indexOf("游泳")!=-1);
	...
	$("#export_user_btn").click(function(){
		//往后端发送请求
		location.href="exportStuday.htm";	
	$('#modal_user_del').modal('hide');   		
	});	
	</script>
